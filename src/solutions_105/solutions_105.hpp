...
class Solution {
   public:
    TreeNode* buildTree(std::vector<int>& preorder, std::vector<int>& inorder) {
        ...
        return helper(inorder_indexes_map, current_index, 0,
                      preorder.size() - 1, preorder, inorder);
    }
    ...
    TreeNode* helper(std::unordered_map<int, int>& inorder_indexes_map,
        ...
        TreeNode* new_node =
            new TreeNode(val,
                         ...
                         helper(inorder_indexes_map, current_index,
                                split_index + 1, right, preorder, inorder));
