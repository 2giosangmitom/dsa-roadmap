...
class Solution {
 private:
  ...
  void dfs(std::vector<std::vector<char>>& board, TrieNode* node, int i, int j,
           std::vector<std::string>& result, int rows, int cols) {
    if (!node->word.empty()) {
    ...
    for (const auto& dir : dirs) {
      ...
      if (c != '#' && node->childrens[c - 'a']) {
        dfs(board, node->childrens[c - 'a'], next_i, next_j, result, rows,
            cols);
      }
    }
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < m; j++) {
        ...
        if (trie.root->childrens[c - 'a']) {
          dfs(board, trie.root->childrens[c - 'a'], i, j, result, n, m);
        }
      }
    }
