    ...
    int calculate(const string &s) {
        ...
        for (char current_char : s) {
            if (isdigit(current_char)) {
                ...
                // Apply the previous number and update the sign for the next
                // number
                result += current_number * sign;
                sign = (current_char == '+') ? 1 : -1;
                current_number = 0;
            } else if (current_char == '(') {
                // Push the current result and sign onto the stack before
                ...
                // Apply the last computed number before exiting the nested
                // expression
                result += current_number * sign;
